...
def _ensure_point(obj):
    if isinstance(obj, Point):
        return obj
    if hasattr(obj, "get_t"):
        return Point(obj.get_t())
    return Point(np.asarray(obj, dtype=float))
...
class GJK:
    ...
    def calculate_distance_and_points(shape0: Support, shape1: Support) -> Tuple[float, Tuple]:
        ...
        while closest_point != origin:
            ...
            vec = -UnitVector(closest_point.get_t())
            point0 = shape0.calculate_support_point(vec)
            point1 = shape1.calculate_support_point(-vec)
            ...
            for point_i in points:
                if np.linalg.norm((point.get_t() - point_i.get_t())) < MathConst.ERROR:
                    finish = True
                    break
            if finish:
                break
            ...
            if np.linalg.norm((closest.get_t() - closest_point.get_t())) < MathConst.EPS:
                break
    ...
    def is_intersecting(shape0: Support, shape1: Support):
        origin = Point([0, 0, 0])
        unit_vector = UnitVector(np.array([1.0, 0.0, 0.0]))

        point = _ensure_point(
            shape0.calculate_support_point(unit_vector)
            - shape1.calculate_support_point(-unit_vector)
        )
        points = [point]
        closest_point = point
        ...
        while closest_point != origin:
            unit_vector = -UnitVector(closest_point.get_t())
            point = _ensure_point(
                shape0.calculate_support_point(unit_vector)
                - shape1.calculate_support_point(-unit_vector)
            )
